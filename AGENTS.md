...
引用事实时使用固定来源 `sources/full_text-20260630.txt:<行号>`、`sources/full_text-20260630-2.txt:<行号>` 或 `sources/full_text-20260713.txt:<行号>`。不要引用 `sources/full_text.txt`，它是 `full_text-20260630.txt` 的移动别名。若需要核对来源元数据，读 [sources/来源-001.md](sources/来源-001.md)、[sources/来源-002.md](sources/来源-002.md) 和 [sources/来源-003.md](sources/来源-003.md)。

`source-001`、`source-002` 承载机制事实；`source-003`（Bodhi Thesis）承载创始人叙事、价值取向和方法论，其中 v1 内容数量、pump.fun 关系、wishpool 事件、VC 联络等属个人回忆，引用时应标注为“创始人回忆，待核实”，不要当作已验证事实。
